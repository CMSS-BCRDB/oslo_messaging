...
# That is required for 'from amqp import ...' statement below.
# Without enforced absolute imports that line tries to import
# local 'oslo_messaging._drivers.amqp' module instead of global
# 'amqp' module in my environment
...
rabbit_opts = [
    ...
    cfg.IntOpt('heartbeat_timeout_threshold',
               deprecated_name='rabbit_heartbeat',
               default=60,
...
class RabbitMessage(dict):
    def __init__(self, raw_message):
        ...
        LOG.debug('RabbitMessage.Init: message %s' % self)
        self._raw_message = raw_message

    def acknowledge(self):
        LOG.debug('RabbitMessage.acknowledge: message %s' % self)
        self._raw_message.ack()

    def requeue(self):
        LOG.debug('RabbitMessage.requeue: message %s' % self)
...
def _try_consume(queue, channel, **kwargs):
    """Consume queue; reconnect, and retry if queue is not found."""
    try:
        queue.consume(**kwargs)
    except amqp_exceptions.NotFound as exc:
        LOG.info(_("The queue %(queue)s was not found, recreate... "
                   "Exception was: %(exception)s"),
                 {"queue": queue, "exception": exc})
        queue.reconnect(channel)
        queue.consume(**kwargs)
        ...
        def _consume():
            ...
            if self.do_consume:
                ...
                for queue in queues_head:
                    _try_consume(queue, self.channel, nowait=True)
                _try_consume(queues_tail, self.channel, nowait=False)

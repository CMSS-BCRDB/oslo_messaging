...
def log_base_exception(infunc):
    def inner_func(*args, **kwargs):
        try:
            return infunc(*args, **kwargs)
        except BaseException:
            logging.exception('Unexpected BaseException occurred '
                              'in RPC Executor!')
            raise
    ...
    def start(self):
        ...
        def _executor_thread():
            try:
                ...
                LOG.exception('Exiting RPC Executor because of GreenletExit')
                return

            LOG.info('Exiting RPC Executor because self._running is %s' %
                     str(self._running))

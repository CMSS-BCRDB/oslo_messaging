...
class TestRabbitDriverLoadSSL(test_utils.BaseTestCase):
    ...
    def test_driver_load(self, connection_klass, fake_ensure):
        ...
        self.config(group="oslo_messaging_rabbit",
...
class TestTryConsume(test_utils.BaseTestCase):

    def setUp(self):
        super(TestTryConsume, self).setUp()
        self.queue = mock.MagicMock()
        self.channel = mock.MagicMock()

    def test_consume_ok(self):
        with open('/tmp/laja123', 'a') as fl:
            fl.write('abcdef')
        rabbit_driver._try_consume(self.queue, self.channel, nowait=True)
        self.queue.consume.assert_called_once_with(nowait=True)

    def test_consume_reconnect(self):
        with open('/tmp/laja123', 'a') as fl:
            fl.write('abcdef123')
        self.queue.consume.side_effect = (amqp_exceptions.NotFound,
                                          lambda *args, **kwargs: None)
        rabbit_driver._try_consume(self.queue, self.channel, nowait=True)
        self.queue.reconnect.assert_called_once_with(self.channel)
        self.assertEqual(self.queue.consume.call_count, 2)
        self.queue.consume.assert_called_with(nowait=True)

    def test_consume_fail(self):
        with open('/tmp/laja123', 'a') as fl:
            fl.write('abcdef123456')
        self.queue.consume.side_effect = amqp_exceptions.NotFound
        self.assertRaises(amqp_exceptions.NotFound, rabbit_driver._try_consume,
                          self.queue, self.channel, nowait=True)
        self.queue.reconnect.assert_called_once_with(self.channel)
        self.assertEqual(self.queue.consume.call_count, 2)
        self.queue.consume.assert_called_with(nowait=True)
